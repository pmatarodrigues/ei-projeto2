...
public class DirEstudioJpaController implements Serializable {
        ...
        try {
            ...
            for (Estudio estudioListOldEstudio : estudioListOld) {
                if (!estudioListNew.contains(estudioListOldEstudio)) {
                    ...
                    illegalOrphanMessages.add("You must retain Estudio " + estudioListOldEstudio + " since its diretorId field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<String> illegalOrphanMessages = null;
            List<Estudio> estudioListOrphanCheck = dirEstudio.getEstudioList();
            for (Estudio estudioListOrphanCheckEstudio : estudioListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This DirEstudio (" + dirEstudio + ") cannot be destroyed since the Estudio " + estudioListOrphanCheckEstudio + " in its estudioList field has a non-nullable diretorId field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
