...
public class AlbumJpaController implements Serializable {
    ...
    public void create(Album album) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            Utilizador editoraId = album.getEditoraId();
            if (editoraId != null) {
                editoraId = em.getReference(editoraId.getClass(), editoraId.getUtilizadorId());
                album.setEditoraId(editoraId);
            }
            Utilizador artistaId = album.getArtistaId();
            if (artistaId != null) {
    ...
    public void edit(Album album) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            Utilizador editoraIdOld = persistentAlbum.getEditoraId();
            Utilizador editoraIdNew = album.getEditoraId();
            Utilizador artistaIdOld = persistentAlbum.getArtistaId();
            Utilizador artistaIdNew = album.getArtistaId();
            List<Musica> musicaListOld = persistentAlbum.getMusicaList();
            List<Musica> musicaListNew = album.getMusicaList();
            if (editoraIdNew != null) {
                editoraIdNew = em.getReference(editoraIdNew.getClass(), editoraIdNew.getUtilizadorId());
